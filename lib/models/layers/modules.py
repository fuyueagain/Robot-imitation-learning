...
class MotionEncoder(nn.Module):
    ...
    def forward(self, x, init, state=None):
        ...
        if state is None:
            ...
            last_pred = init[..., :self.n_joints * 3]
        else:
            h0, last_pred = state
        pred_list = [last_pred]
        motion_context_list = []
        ...
        return pred_kp3d, motion_context, (tuple([h.detach() for h in h0]) if isinstance(h0, tuple) else h0.detach(), pred_list[-1].detach())
...
class TrajectoryDecoder(nn.Module):
    ...
    def forward(self, x, root, cam_a, h0=None, state=None):
        ...
        if state is None:
            ...
            h0, last_root = state
        pred_root_list, pred_vel_list = [last_root], []
        ...
        return pred_root, pred_vel, (tuple([h.detach() for h in h0]) if isinstance(h0, tuple) else h0.detach() if h0 is not None else None, pred_root_list[-1].detach())
...
class MotionDecoder(nn.Module):
    ...
    def forward(self, x, init, state=None):
        ...
        if state is None:
            ...
            last_pose = init.reshape(b, 1, -1)
        else:
            h0, last_pose = state
        
        # Recursive prediction of SMPL parameters
        pred_pose_list = [last_pose]
        ...
        return pred_pose, pred_shape, pred_cam, pred_contact, (tuple([h.detach() for h in h0]) if isinstance(h0, tuple) else h0.detach(), pred_pose_list[-1].detach())
